...
public class UI {
	...
	GamePanel gp;
	Graphics2D g2;
	public Font maruMonica, poppinsThin;
	...
	public UI(GamePanel gp) {
		...
		try {
			...
			is = getClass().getResourceAsStream("/fonts/Poppins-Thin.ttf");
			poppinsThin = Font.createFont(Font.TRUETYPE_FONT, is);
	...
	public void trade_select() {
		...
		if (commandNum == 2) {
			...
			if (gp.keyH.enterPressed == true) {
				...
				currentDialogue = "I'll be waiting for you, hehe!";
			}
		}
	}
	public void trade_buy() {
		...
		drawSubWindow(x, y, width, height);
		g2.drawString("Coins Held: " + gp.player.coin, x+24, y+60);
		...
		if (itemIndex < npc.inventory.size()) {
			...
			if (gp.keyH.enterPressed == true) {
				if(npc.inventory.get(itemIndex).price > gp.player.coin) {
					subState = 0;
					gp.gameState = gp.dialogueState;
					currentDialogue = "You don't have enough coins to buy that...";
					drawDialogueScreen();
				}
				else {
	...
	public void drawSleepScreen() {
		...
		if (counter >= 120) {
			gp.environmentManager.lighting.filterAlpha -= 0.01f;
			if (gp.environmentManager.lighting.filterAlpha <= 0) {
